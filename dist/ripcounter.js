...
var RipCounter = /** @class */ (function () {
    function RipCounter(localStore) {
        ...
        if (this.store.has('ripcounter_settings')) {
            ...
            var ripsettings_json = JSON.parse(this.store.get('ripcounter_settings'));
    ...
    RipCounter.prototype.getRipsByGameSearch = function (searchName) {
        var resultList = [];
        this.RipCounterMap.forEach(function (value, key) {
            //console.log('key: ' + key);
            if (key.toLowerCase().includes(searchName.toLocaleLowerCase()) && resultList.length < 10) {
                var retStr = key + ' - Deaths: ' + value[0] + ' Gravity-Deaths: ' + value[1];
        ...
        if (this.ripcounterSettings.active) {
            ...
            if (this.ripcounterSettings.rip_command_alias.includes(message.toLowerCase())) { //RIP
                ...
                if (message.toLowerCase().split(' ').length > 1) {
                    var ripserach_message = this.getRipsByGameSearch(message.toLowerCase().split(' ')[1]);
